...
namespace UNiDAYS.Testing.NUnit
{
	public static class ShouldExtensions
	{
		...
		{
			actual.ShouldMatch(regex, RegexOptions.None);
		}

		public static void ShouldMatch(this string actual, string regex, RegexOptions options)
		{
			var reg = new Regex(regex, options);

			Assert.IsTrue(reg.IsMatch(actual), "`" + actual + "` does not match regex `" + regex + "`");
		}
	}
}